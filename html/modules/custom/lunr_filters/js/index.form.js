...
(function ($, Drupal) {
   ...
   *   An optional callback.
   */
  function indexPage(builder, path, uploadPath, displayField, usePager, page, indexFields, callback) {
    $.ajax({
      url: path + '?page=' + page,
      type : 'GET',
      dataType: 'json',
      success: function(data) {
        ...
        data.forEach(function(row) {
          ...
          for (var field in indexFields) {
            if (indexFields.hasOwnProperty(field) && row.hasOwnProperty(field)) {
              documentRow[field] = row[field];
            }
          }
  ...
  function indexNextPath(paths, uploadPaths, indexFields, displayField, usePager, callback) {
    ...
    last_response = null;
    indexPage(builder, path, uploadPath, displayField, usePager, 0, indexFields, function (success) {
      if (success) {
    ...
    attach: function attach(context, settings) {
      $('input.js-lunr-search-index-button:not(disabled)', context).once('lunr-search-index-form').on('click', function (e) {
        ...
        indexNextPath(paths, uploadPaths, indexSettings.indexFields, indexSettings.displayField, indexSettings.usePager, function () {
          ...
          $button.addClass('lunr-search-indexing-complete');
